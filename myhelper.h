...
public:
    ...
    static int addHeadLen(void* data, int datalen, int headLen = 2)
    {
        ...
        char tmp[MAXLEN + 1] = { 0 };
        int len = datalen;
        for (int i = MAXLEN; i>(MAXLEN - headLen); i--)
        {
            tmp[i] = len % 0xff;
            len /= 0xff;
        }
        memmove(pdata + headLen, pdata, datalen);
        memcpy(pdata, tmp + MAXLEN - headLen + 1, headLen);
        return (datalen + headLen);
    }

    //获取报文头长度域
    static int getHeadLen(void* header, int len)
    {
        if( NULL==header || len>4 )
            return -1;

        char* pdata = (char*)header;
        for(int i=0;i<len;i++){
            len += ( pdata[i] * pow(256,len-i-1) );
        }
        return len;
    }
    ...
    static inline char hextoasc(int xxc)
    {
        xxc&=0x0f;
        if (xxc<0x0a) xxc+='0';
        else xxc+=0x37;
    ...
    static inline char hexlowtoasc(int xxc)
    {
        xxc&=0x0f;
        if (xxc<0x0a) xxc+='0';
        else xxc+=0x37;
    ...
    static inline char hexhightoasc(int xxc)
    {
        xxc&=0xf0;
        xxc = xxc>>4;
        if (xxc<0x0a) xxc+='0';
    ...
    static inline char asctohex(char ch1,char ch2)
    {
        char ch;
    ...
    static int aschex_to_bcdhex(const char* aschex,int len, char* bcdhex)
    {
        int i,j;
        if (len % 2 == 0)
            j = len / 2;
    ...
    static int bcdhex_to_aschex(const char* bcdhex,int len,char* aschex)
    {
        int i;
        for (i=0;i<len;i++)
        {
